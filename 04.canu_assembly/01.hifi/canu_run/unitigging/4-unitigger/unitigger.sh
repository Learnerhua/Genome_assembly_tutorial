...
if [ -e scer.ctgStore/seqDB.v001.tig ]; then
  exists=true
else
  exists=false
fi
...
if [ -e ./scer.ctgStore ] ; then
  mv ./scer.ctgStore ../scer.ctgStore
fi
...
#
if [ -e ../scer.ctgStore -a \
   ! -e ../scer.ctgStore/seqDB.v001.sizes.txt ] ; then
  $bin/tgStoreDump \
    -S ../../scer.seqStore \
    -T ../scer.ctgStore 1 \
    -sizes -s 12000000 \
   > ../scer.ctgStore/seqDB.v001.sizes.txt
fi

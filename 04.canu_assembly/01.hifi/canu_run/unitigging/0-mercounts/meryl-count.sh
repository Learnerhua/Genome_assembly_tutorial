...
if [ -e ./scer.$jobid.meryl/merylIndex ] ; then
...
if [ -e scer.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
${DATA_ROOT}/Download/canu-2.3/bin/meryl k=22 threads=4 memory=3 \
  count \
    segment=$jobid/01 ../../scer.seqStore \
    output ./scer.$jobid.meryl.WORKING \
&& \
mv -f ./scer.$jobid.meryl.WORKING ./scer.$jobid.meryl
